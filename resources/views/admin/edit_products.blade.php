        ...
        <div class="card shadow">
            ...
            <div class="card-body">
                <form action="" method="POST" enctype="multipart/form-data" id="form-submit">
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            <img class="rounded me-2" alt="200x200" width="200" src="{{ asset('uploads/products/' . $product->image) }}" data-holder-rendered="true">
                        </div>
                    </div>
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            ...
                            <input type="text" name="product_name" id="product_name" placeholder="Enter product name" oninput="fieldcheck('#product_name', '#err_product_name')" value="{{ $product->product_name }}" class="form-control" required>
                    ...
                    <div class="row mb-3">
                        <div class="col-md-12">
                            ...
                            <textarea name="description" id="description" placeholder="Enter product description" oninput="fieldcheck('#description', '#err_description')" class="form-control" rows="6" required>{{ $product->description }}</textarea>
                    ...
                    <div class="row mb-3">
                        <div class="col-md-4">
                            ...
                            <input type="number" name="base_price" id="base_price" placeholder="Enter base price" oninput="fieldcheck('#base_price', '#err_base_price')" value="{{ $product->base_price }}" class="form-control" step="0.01" required>
                            <span class="error_field" id="err_base_price"></span>
                        </div>

                        <div class="col-md-4">
                            <label for="start_date" class="form-label">Starting Date & Time</label>
                            <input type="datetime-local" name="start_date" id="start_date" class="form-control" oninput="fieldcheck('#start_date', '#err_start_date')" value="{{ $product->start_date }}" required>
                            <span class="error_field" id="err_start_date"></span>
                        </div>

                        <div class="col-md-4">
                            <label for="end_date" class="form-label">Ending Date & Time</label>
                            <input type="datetime-local" name="end_date" id="end_date" class="form-control" oninput="fieldcheck('#end_date', '#err_end_date')" value="{{ $product->end_date }}" min="{{ $product->start_date }}" required>
                            <span class="error_field" id="err_end_date"></span>
                        </div>
                    </div>

                    <div class="text-center mt-4">
                        <button type="button" class="btn btn-primary" id="submitBtn">Update Product</button>
    ...
    $(document).on("click", "#submitBtn", function(e){
        ...
        if(errProduct == false && errDesc == false && errBprice == false && errSdate == false && errEdate == false){
            ...
            $.ajax({
                method:"POST",
                dataType : "json",
                url : "{{ route('update_products', ['id' => $product->id]) }}",
                data : formData,
                processData: false,
                contentType: false,
                beforeSend:function(){
